...
function grad4D(hash: number, x: number, y: number, z: number, w: number): number {
...
function perlinNoise4D(x: number, y: number, z: number, w: number): number {
  ...
  const x1 = lerp(grad4D(aaaa, xf, yf, zf, wf), grad4D(baaa, xf - 1, yf, zf, wf), u);
  const x2 = lerp(grad4D(aaba, xf, yf, zf - 1, wf), grad4D(baba, xf - 1, yf, zf - 1, wf), u);
  const x3 = lerp(grad4D(abaa, xf, yf - 1, zf, wf), grad4D(bbaa, xf - 1, yf - 1, zf, wf), u);
  const x4 = lerp(grad4D(abba, xf, yf - 1, zf - 1, wf), grad4D(bbba, xf - 1, yf - 1, zf - 1, wf), u);
  ...
  const x5 = lerp(grad4D(aaab, xf, yf, zf, wf - 1), grad4D(baab, xf - 1, yf, zf, wf - 1), u);
  const x6 = lerp(grad4D(aabb, xf, yf, zf - 1, wf - 1), grad4D(babb, xf - 1, yf, zf - 1, wf - 1), u);
  const x7 = lerp(grad4D(abab, xf, yf - 1, zf, wf - 1), grad4D(bbab, xf - 1, yf - 1, zf, wf - 1), u);
  const x8 = lerp(grad4D(abbb, xf, yf - 1, zf - 1, wf - 1), grad4D(bbbb, xf - 1, yf - 1, zf - 1, wf - 1), u);
...
export function Noise() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Calculate grid
      const rows = Math.ceil(dims.displayHeight / dims.cellHeight) + 2;
      ...
      for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
          ...
          const charIndex = Math.max(0, Math.min(charsByBrightness.length - 1, rawIndex));
